...
export class OpenAIRealtimeService {
  ...
  sendSessionUpdate() {
    const prompt = this.mode === 'outbound' ? SARAH_OUTBOUND : SARAH_INBOUND;
        ...
        tools: [
          {
            ...
            description: "Schedule a technical assessment after getting The Trifecta (contactName, companyName, verified phone) and a specific time.",
            parameters: {
              type: "object",
              properties: {
          ...
          },
          {
            ...
            description: "Report the interaction if the user is not interested, asks to call later, or if it is a voicemail.",
            parameters: {
              type: "object",
              properties: {
    ...
    // Initial greeting for outbound
    if (this.mode === 'outbound') {
      setTimeout(() => {
        this.sendToOpenAI({
          type: 'response.create',
          response: {
            instructions: "Start the conversation immediately by greeting them and asking if they handle the technology or if you should speak to the Office Manager."
          }
        });
      }, 500);
    } else {
        ...
        this.sendToOpenAI({
          type: 'response.create',
          response: {
            instructions: "Start the conversation immediately by greeting them and asking how you can help them today."
          }
        });
      }, 500);
    }
  ...
  async handleFunctionCall(event) {
    ...
    try {
      if (name === 'schedule_appointment') {
        ...
        sendEmailReport('REPORT', interaction);
        ...
        }, 10000); // 10s delay to allow goodbye
      }
