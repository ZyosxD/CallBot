...
export const validateTwilioRequest = async (request, reply) => {
  ...
  if (!config.server.publicUrl || config.server.publicUrl.includes('localhost') || config.server.publicUrl.includes('ngrok')) {
    return; // Fastify preHandler returns nothing on success
  }

  const twilioSignature = request.headers['x-twilio-signature'];
  const url = config.server.publicUrl + request.raw.url; // Fastify uses request.raw.url
  ...
  if (!requestIsValid) {
    ...
    return reply.code(403).send('Forbidden'); // Fastify way to halt
  }
};
